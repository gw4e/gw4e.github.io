...
you link them by setting a comma separated list of requirements in vertices properties of the graph model. These informations, can be used to create 
traceability with external requirements tooling such as [Squash](http://www.squashtest.org/fr). The information associated to the requirements are available at the end of the test execution in a JSON format.
